...
public class ScenesScenario extends AbstractScenario {
	...
	private int numberOfAmbulances = 7;

	private int percentageOfMibleComEnabledAgents = 50;
	...
	protected OutputInterface getOutput() {
		...
		specialAgents.add("AmbulanceRole");
		specialAgents.add("FireEngineRole");
		specialAgents.add("TeamFirefighterRole");
		specialAgents.add("TeamRescuerRole");
		specialAgents.add("RescueVehicleRole");
	...
	public ScenesScenario() {
		...
		percentageOfMibleComEnabledAgents = 50;
		...
		if (doMultipleRuns) {
			System.err
					.println("SIMREIHE GESTARTET, Consolenausgabe deaktiviert.");
		...
		System.out.println("####### agents," + numberOfAgents
				...
				+ percentageOfMibleComEnabledAgents + "; seed," + initialSeed
				+ " ##########");
		...
		if (doMultipleRuns) {
			...
			for (int i = 2; i <= amountOfSimulationRuns; i++) {
				...
				System.out.println("####### agents," + numberOfAgents
						...
						+ percentageOfMibleComEnabledAgents + "; seed," + seed
						+ " ##########");
				System.err.println("####### agents," + numberOfAgents
						+ "; run," + i + " - MobComEnabledPercentage,"
						+ percentageOfMibleComEnabledAgents + "; seed," + seed
		...
		System.err.println("All simulations terminated");
	...
	public void preRun() {
		...
		for (int i = 0; i < numberOfAgents; i++) {
			a = createAgentInGreen((Scheduler.rand.nextInt(100)) < percentageOfMibleComEnabledAgents);
			// a = createAgentOnMap();
	...
	public void preCycle() {
		...
		// An ambulance every 2 minutes
		if (ambulanceCounter == 120 && fireCounter < 3601) {
			ambulanceCounter = 0;
			a = createAgentOnBorder();
			a.setRole(new AmbulanceRole(a));
			Scheduler.agentRepository.put(a);
